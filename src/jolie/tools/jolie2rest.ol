...
define __config_operation {
    ...
    file_content = file_content + "\troutes[" + __r_counter + "] << {\n";
...
define __renderize {
    ...
    file_content = file_content + "}";
    undef( file );
    file.filename = "router_import.ol";
...
main {
    ...
    for( l = 0, l < #lines.result, l++ ) {
      ...
      trim@StringUtils( params.result[ 0 ] )( service_filename );
      trim@StringUtils( params.result[ 1 ] )( service_input_port );
                                                        ...
                                                        if ( find_str_res == 1 ) {
                                                            if ( is_defined( current_sbt.type_inline ) ) {
                                                                  if ( #current_sbt.type_inline.sub_type > 0 ) {
                                                                      println@Console( error_prefix +  "but the field " + current_sbt.name + " has a type with subnodes which is not permitted")();
                                                                      throw( DefinitionError )
                                                                  };

                                                                  if ( is_defined( current_sbt.type_inline.root_type.type_link ) ) {
                                                                    println@Console( error_prefix +  "but the field " + current_sbt.name + " cannot reference to another type because it is a path parameter")();
                                                                    throw( DefinitionError )
                                                                  }
                                                            }
                                                        }
      ...
      if ( swagger_enable ) {
          ...
          scope( j2s ) {
              ...
              f.filename = "swagger_" + service_input_port + ".json";
              writeFile@File( f )();
